...
(() => {
  ...
  function loadTranslationLib() {
    ...
    const yandex = {
      needkey: !0,
      fetch: ({ key: e, from: from, to: to, text: text }) => [
      ...
      parse: (res) =>
        res.json().then((body) => {
          if (200 !== body.code) throw new Error(body.message);
          return body.text[0];
        }),
    };
    const libre = {
      needkey: !1,
      fetch: ({ url: e = libreUrl, key: key, from, to: to, text: text }) => [
        e,
        {
          method: "POST",
    ...
    var languages = (e) => {
      if ("string" != typeof e)
        throw new Error(
          'The "language" must be a string, received ' + typeof e
        );
      if (e.length > 100)
    ...
    const Translate = function (e = {}) {
      ...
      const defaults = {
        ...
        translate = async (text, opts = {}) => {
          "string" == typeof opts && (opts = { to: opts });
